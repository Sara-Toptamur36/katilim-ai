...
test calistirmasi, docs/donanim_tanilama_raporu.md'ye komsu bulgu) -
**6/7'si (%86) dogru sekilde reddedildi**. Kacan tek vaka (TOM-002,
odul_miktari=2500) gercekten sinirda: kaynakta "kampanya donemi boyunca
kazanilabilecek iade 2.500 TL ile sinirlidir" cumlesi var - sayi da
baglam kelimesi de (iade) GERCEKTEN oradaydi, sadece bu bir "donem
ustu tavan" degeri, tek islemlik "odul miktari" degil - insan
etiketleyici icin bile tartisilabilir bir sinir vakasi.
...
yakin dustugu (yukaridaki TOM-002 gibi) durumlarda yanlis pozitif
(hatali "dogrulandi") verebilir. Amac halusinasyonu SIFIRLAMAK degil,
...
def _cumle_penceresi(metin_kucuk: str, baslangic: int, bitis: int) -> str:
    ...
    AYRI, ilgisiz cumlede. Sadece karakter penceresi bunu yanlislikla
    'ayni baglamda' sayardi; cumle siniri bu tesadufi yakinligi keser."""
    onceki_sinir = max(
        (metin_kucuk.rfind(c, 0, baslangic) for c in ".!\n"), default=-1
    )
...
def sayisal_iddiayi_dogrula(alan_adi: str, deger: float, kaynak_metin: str) -> DogrulamaSonucu:
    ...
    for varyant in varyantlar:
        ...
        for eslesme in desen.finditer(metin_kucuk):
            ...
            if any(kelime.lower() in pencere for kelime in baglam_kelimeleri):
                baglamda_dogrulandi = True
                break
